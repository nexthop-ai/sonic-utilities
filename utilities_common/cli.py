...
class ShowVrfGroup(click.Group):
    def format_usage(self, ctx, formatter):
        formatter.write_usage(ctx.command_path, "[COMMAND|VRF_NAME]")

    def parse_args(self, ctx, args):
        args = list(args)
        valid_commands = ['-h', '--help'] + list(self.commands.keys())
        vrf_name = None
        if args and args[0] not in valid_commands:
            vrf_name = args.pop(0)
        ctx.params['vrf_name'] = vrf_name
        return super().parse_args(ctx, args)
