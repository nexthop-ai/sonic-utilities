...
=======
    ...
    def test_add_portchannel_default_lacp_mode(self):
        ...
        # add a portchannel without specifying lacp mode, should default to 'coupled'
        result = runner.invoke(config.config.commands["portchannel"].commands["add"], ["PortChannel0005"], obj=obj)
        print(result.exit_code)
        print(result.output)
        assert result.exit_code == 0
        assert db.cfgdb.get_entry('PORTCHANNEL', 'PortChannel0005')['lacp_mode'] == 'coupled'
...
>>>>>>> d16220bc (NOS-10747: Minor fix for test (#772))
