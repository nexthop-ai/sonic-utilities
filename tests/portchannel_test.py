...
<<<<<<< HEAD
=======
    @pytest.mark.parametrize("lacp_mode", ["coupled", "independent", "COUPLED", "Independent"])
    ...
    def test_add_portchannel_default_lacp_mode(self):
        ...
        # add a portchannel without specifying lacp mode: the field must be
        # omitted so teammgrd inherits the switch-wide DEVICE_METADATA
        # default_lacp_mode
        result = runner.invoke(config.config.commands["portchannel"].commands["add"], ["PortChannel0005"], obj=obj)
        print(result.exit_code)
        print(result.output)
        assert result.exit_code == 0
        assert 'lacp_mode' not in db.cfgdb.get_entry('PORTCHANNEL', 'PortChannel0005')

    @pytest.mark.parametrize("lacp_mode", ["coup", "indep"])
