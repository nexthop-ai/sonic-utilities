...
class TestTamIfa(object):
    @classmethod
    def setup_class(cls):
        os.environ['UTILITIES_UNIT_TESTING'] = "1"
        print("SETUP")

    def test_tam_ifa_text_output(self):
        """Test 'show tam ifa' with table output."""
        runner = CliRunner()
        db = Db()

        result = runner.invoke(show.cli.commands["tam"].commands["ifa"], obj=db)
        ...
        assert "enterprise-id" in result.output
    ...
    def test_tam_ifa_json_output(self):
        """Test 'show tam ifa --json' with JSON output."""
        runner = CliRunner()
        db = Db()

        result = runner.invoke(
            show.cli.commands["tam"].commands["ifa"],
            ["--json"],
            obj=db
        )
        ...
        assert output["device-id"] == "12345"
        assert output["enterprise-id"] == "54321"
        assert output["ifa"] == "true"
        # Only 3 fields should be present
        assert len(output) == 3

    @mock.patch('show.tam.TamIfaShow.get_ifa_config', return_value={})
    def test_tam_ifa_empty_text_output(self, mock_get_ifa_config):
        """Test 'show tam ifa' with no configuration (text output)."""
        runner = CliRunner()
        db = Db()
        ...
        assert result.exit_code == 0
        # When no config, table displays headers with empty values
        assert "Field" in result.output
        assert "Value" in result.output
        assert "device-id" in result.output
        assert "enterprise-id" in result.output
    ...
    @mock.patch('show.tam.TamIfaShow.get_ifa_config', return_value={})
    def test_tam_ifa_empty_json_output(self, mock_get_ifa_config):
        """Test 'show tam ifa --json' with no configuration (JSON output)."""
        runner = CliRunner()
        db = Db()
        ...
        # Parse JSON output - should be a dict with empty string values
        output = json.loads(result.output)
        assert isinstance(output, dict)
        assert output["device-id"] == ""
        assert output["enterprise-id"] == ""
        assert output["ifa"] == ""
        # Only 3 fields should be present
        assert len(output) == 3
