...
<<<<<<< HEAD
=======
    def assert_correct_multiline_desc(self, output, vrf_table, vrf, line_width=40):
        attrs = vrf_table.get(vrf, {})
        assert attrs
        desc = attrs['description']
        assert desc
        vrf_name_index = output.find(vrf)

        # check long descriptions are broken into multiple lines
        # AND comes after its associated VRF name
        for i in range(0, len(desc), line_width):
            line_end = min(len(desc), i + line_width)
            line = desc[i:line_end].strip()
            assert output.find(line) > vrf_name_index

    def test_vrf_show_correct_columns(self):
        """Test show VRF command displays VRF name, description, and interfaces"""
        runner = CliRunner()
        db = Db()
    ...
    def test_vrf_show_no_custom_vrfs(self, default_vrf_only):
        """Test show VRF command exits with a message saying all interfaces in default VRF"""
        runner = CliRunner()
        result = runner.invoke(show.cli.commands['vrf'])
        assert result.exit_code == 0
        assert "All interfaces are in default VRF.\n" == result.output

    def test_vrf_show_no_custom_vrfs_filter(self, default_vrf_only):
        """Test show VRF command fails when no custom VRFs exist and interface is specified"""
        runner = CliRunner()
        vrf_name = "Vrf-null"
        result = runner.invoke(show.cli.commands['vrf'], [vrf_name])
        assert result.exit_code != 0
        assert f"{vrf_name} is not configured." in result.output

>>>>>>> d814eb73 (NOS-7311: add missing test case for "show vrf summary" (#460))
