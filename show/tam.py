...
- show tam ifa: Display TAM IFA configuration
"""
...
# CONFIG_DB table name for TAM device configuration
CONFIG_TAM_TABLE = "TAM"
TAM_DEVICE_KEY = "device"
...
class TamIfaShow:
    """Class to handle TAM IFA (Inband Flow Analyzer) show commands."""

    def __init__(self):
        self.config_db = ConfigDBConnector()
        self.config_db.connect()

    def get_ifa_config(self):
        """Get TAM IFA configuration from CONFIG_DB."""
        data = self.config_db.get_entry(CONFIG_TAM_TABLE, TAM_DEVICE_KEY)
        return data if data else {}

    def format_table_output(self, config):
        """Format IFA configuration as a table."""
        # Define table headers
        header = ["Field", "Value"]

        body = []
        # Add rows for each field in the requested order
        fields = ["device-id", "enterprise-id", "ifa"]
        for field in fields:
    ...
    def format_json_output(self, config):
        """Format IFA configuration for JSON output."""
        # Keep only the requested fields in order
        output = {
            "device-id": config.get("device-id", ""),
            "enterprise-id": config.get("enterprise-id", ""),
            "ifa": config.get("ifa", "")
        }
        return output
...
@click.option('--json', '-j', 'json_output', is_flag=True, default=False,
              help="Display the output in JSON format")
def ifa(json_output):
    """Show TAM IFA configuration"""
    try:
        tam_ifa_show = TamIfaShow()
        ifa_config = tam_ifa_show.get_ifa_config()

        if json_output:
            output = tam_ifa_show.format_json_output(ifa_config)
            click.echo(json_module.dumps(output, indent=2))
        else:
            header, body = tam_ifa_show.format_table_output(ifa_config)
            click.echo(tabulate(body, header))

    except Exception as e:
        click.echo(f"Error retrieving TAM IFA configuration: {e}", err=True)
        raise click.Abort()
